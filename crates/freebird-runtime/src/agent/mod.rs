...
use freebird_traits::provider::{
    CompletionRequest, CompletionResponse, ContentBlock, Message, Role, ToolDefinition,
};
...
pub enum RuntimeError {
    ...
    Provider(#[from] freebird_traits::provider::ProviderError),
...
pub struct AgentRuntime {
    ...
    pub(crate) tool_executor: crate::tool_executor::ToolExecutor,
    ...
    pub(crate) approval_rx: std::sync::Mutex<Option<tokio::sync::mpsc::Receiver<ApprovalRequest>>>,
    pub(crate) memory: Arc<dyn Memory>,
    pub(crate) knowledge_store: Option<Arc<dyn KnowledgeStore>>,
    pub(crate) knowledge_config: KnowledgeConfig,
    ...
    pub(crate) default_session_ttl_hours: u64,
    pub(crate) event_sink: Option<Arc<dyn EventSink>>,
    pub(crate) audit_sink: Option<Arc<dyn AuditSink>>,
    pub(crate) summary_store: Option<Arc<dyn SummarySink>>,
    pub(crate) summarization_config: SummarizationConfig,
...
pub(crate) async fn send_outbound(outbound: &mpsc::Sender<OutboundEvent>, event: OutboundEvent) {
...
mod tests {
    ...
    use freebird_traits::knowledge::{
        KnowledgeEntry, KnowledgeKind, KnowledgeMatch, KnowledgeSource,
    };
